...
export function DataField({ icon: Icon, label, value }: DataFieldProps) {
    return (
        <div className="flex flex-col w-full gap-2">
            ...
            <div className="w-full gap-2 sm:gap-3 min-h-12 items-center flex flex-row p-1 sm:p-2 px-3 sm:px-4 h-fit bg-card rounded-lg sm:rounded-xl border">
                <Icon className="min-w-4 h-4 sm:min-w-5 sm:h-5 text-muted-foreground flex-shrink-0" />
...
export function DictionaryField({ label, values }: DictionaryFieldProps) {
    return (
        <div className="flex flex-col w-full gap-2">
            <Label className="text-sm sm:text-base">{label}</Label>
            <div className="w-full gap-2 sm:gap-3 items-center flex flex-row p-3 sm:p-4 min-h-12 flex-wrap bg-card rounded-lg sm:rounded-xl border">
